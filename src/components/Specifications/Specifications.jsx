...
export default function Specifications() {
  ...
  return (
    <section className="py-24 bg-ev-darker">
          ...
          <div className="w-full md:w-1/3 flex flex-row md:flex-col overflow-x-auto hide-scrollbar gap-2 md:gap-0">
            {tabs.map((tab) => (
              <button
                ...
                className={`relative px-6 py-4 text-left font-medium transition-colors whitespace-nowrap md:whitespace-normal md:border-l-2 ${
                  activeTab === tab 
                    ? 'text-white border-ev-accent md:bg-white/5' 
              ...
              >
                {activeTab === tab && (
                  <motion.div 
                    ...
                    className="absolute left-0 top-0 bottom-0 w-0.5 bg-ev-accent hidden md:block"
                  />
                )}
                {tab}
              </button>
              ...
              >
                {specifications[activeTab].map((item, idx) => (
                  <div key={idx} className="border-b border-white/10 pb-4">
                    <p className="text-white/50 text-sm tracking-wider uppercase mb-2">{item.label}</p>
