  ...
  return (
    ...
    >
      <motion.div
        ...
        className="absolute right-0 top-0 bottom-0 w-full md:w-[450px] bg-[#0a0a0a] border-l border-white/10 p-8 flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute top-8 right-8 w-10 h-10 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="mt-16 flex flex-col h-full">
          <span className="text-ev-accent text-sm font-medium mb-2">{detail.number}</span>
          <h3 className="text-3xl font-semibold text-white mb-6 leading-tight">{detail.title}</h3>
          
          <div className="aspect-video w-full rounded-xl overflow-hidden mb-8 border border-white/5 bg-[#111]">
            <ImageWithFallback
